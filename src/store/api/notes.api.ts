...
export const notesApi = baseApi.injectEndpoints({
  endpoints: (build) => ({
    applicationNotes: build.query<Note[], string>({
      query: (id) => `/applications/${id}/notes`,
      transformResponse: unwrap,
      providesTags: (_r, _e, id) => [{ type: "Notes", id }],
    }),
    createNote: build.mutation<Note, { applicationId: string; body: CreateNoteInput }>({
      ...
      invalidatesTags: (_r, _e, { applicationId }) => [
        "Applications",
        { type: "Notes", id: applicationId },
        { type: "Application", id: applicationId },
      ],
    }),
    updateNote: build.mutation<Note, { id: string; applicationId: string; body: CreateNoteInput }>({
    ...
    deleteNote: build.mutation<void, { id: string; applicationId: string }>({
